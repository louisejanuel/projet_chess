    ...
    void render(Shader& shader, const Chessboard& logicBoard, const std::vector<int>& states, int animFrom = -1, int animTo = -1, float animProgress = 1.0f)
    {
        ...
        for (int x = 0; x < 8; ++x)
        {
            for (int z = 0; z < 8; ++z)
            {
                ...
                glm::vec3 baseColor;
                if ((x + z) % 2 == 0)
                    baseColor = glm::vec3(0.9f, 0.8f, 0.7f);
                else
                    baseColor = glm::vec3(0.4f, 0.2f, 0.1f);

                if (state == 2)
                    baseColor = glm::vec3(0.4f, 0.8f, 0.4f);
                else if (state == 1)
                    baseColor = glm::vec3(0.8f, 0.8f, 0.4f);
                else if (state == 3)
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (i == 0)
                borderModel = glm::translate(borderModel, glm::vec3(0.0f, 0.0f, -offset));
            else if (i == 1)
                borderModel = glm::translate(borderModel, glm::vec3(0.0f, 0.0f, offset));
            else if (i == 2)
                borderModel = glm::translate(borderModel, glm::vec3(offset, 0.0f, 0.0f));
            else if (i == 3)
                borderModel = glm::translate(borderModel, glm::vec3(-offset, 0.0f, 0.0f));

            if (i < 2)
                borderModel = glm::scale(borderModel, glm::vec3(8.0f + 2.0f * thickness, height, thickness));
            else
                borderModel = glm::scale(borderModel, glm::vec3(thickness, height, 8.0f));
        ...
        for (int i = 0; i < 64; ++i)
        {
            if (!logicBoard.is_empty(i))
            {
                ...
                if (piece->get_color() == Color::White)
                    pieceColor = glm::vec3(0.9f, 0.9f, 0.9f);
                else
                    pieceColor = glm::vec3(0.2f, 0.2f, 0.2f);

                if (state == 2)
                    pieceColor += glm::vec3(0.0f, 0.4f, 0.0f);
                else if (state == 3)
                    pieceColor += glm::vec3(0.2f, 0.2f, 0.2f);
