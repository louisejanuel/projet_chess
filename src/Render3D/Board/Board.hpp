    ...
    void render(Shader& shader, const Chessboard& logicBoard) {
        for (int x = 0; x < 8; ++x) {
            for (int z = 0; z < 8; ++z) {
                ...
                if ((x + z) % 2 == 0) shader.setVec3("baseColor", glm::vec3(0.9f, 0.8f, 0.7f));
                else shader.setVec3("baseColor", glm::vec3(0.4f, 0.2f, 0.1f));
        ...
        for (int i = 0; i < 64; ++i) {
            if (!logicBoard.is_empty(i)) {
                ...
                if (type == Type::Pawn) pawnModel.draw();
                else if (type == Type::Rook) rookModel.draw();
                else if (type == Type::Bishop) bishopModel.draw();
                else if (type == Type::Knight) knightModel.draw();
                else if (type == Type::Queen) queenModel.draw();
                else if (type == Type::King) kingModel.draw();
            }
        }
    }
};